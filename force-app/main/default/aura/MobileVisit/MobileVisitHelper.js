                    ...
                    if(component.get('v.checkout')==='true'){
                        ...
                        component.set('v.checkout','false');
                         component.set('v.spinner',false);
                          ...
                          console.log('checkin:'+component.get('v.checkin'))
                        helper.showToast("Successfully checked in","Success");
                        
                        component.set('v.checkin','false');
                        
                    }else if(component.get('v.newvisit')==='true'){
                        console.log('newvisit:'+component.get('v.newvisit'))
                        if(component.get('v.RevisitEID')==""){
                            helper.updateReVisit(component,event,helper);
                        }else{
                            helper.updateCurrentVisit(component,event,helper);
...
//        }
    ...
    updateCurrentVisit : function(component,event,helper){
        ...
        console.log('Current Visit Data:===='+JSON.stringify(component.get('v.data.currentvisit')))
    },
    ...
    //Validating Order/Invoice before Saving
    ...
    saveUnsealed : function (component, event, helper) {
      /*  
        ...
        for(var i=0; i<data.orderitems.length; i++){
            for(var j=0; j<data.orderitems[i].length; j++){
                ...
                if(data.orderitems[i][j].Quantity__c > data.orderitems[i][j].Available_Quantity__c){
                    ...
                    component.set('v.disableUnsealButton',false);
                    component.set('v.spinner',false);
                    break;
                }else{
                   stockAvailable = true; 
       ...
       orderitems.forEach(function(record){
         
               var manufactureDate =record.Stock__c;
               if (record.Quantity__c==null || record.Quantity__c == '' || record.Quantity__c <= 0) {
                   helper.showToast('Please Enter  Quantity for '+record.Name, "error");
                   stockAvailable = false;
                   component.set('v.disableUnsealButton',false);
                   component.set('v.spinner',false);
               } else  if(record.Quantity__c > record.Available_Quantity__c){
                   stockAvailable = false;
                   helper.showToast("Added quantity is exceeding then available quantity for "+'"'+record.Name+'"',"error");
                   component.set('v.disableUnsealButton',false);
                   component.set('v.spinner',false);
               }else if (manufactureDates.has(manufactureDate)) {
                   helper.showToast('Duplicate entry '+record.Name,  "error" );
                   stockAvailable = false;
                   component.set('v.disableUnsealButton',false);
                   component.set('v.spinner',false);
               } else if (manufactureDate) {
                   manufactureDates.add(manufactureDate); // Add unique manufacture date
               }
           });
        ...
        if(stockAvailable != false){
            try{
             ...
             var prods = data.orderitems;
                ...
                action1.setParams({'orderList':  JSON.stringify(data.orders),
                                   'accountId': accId,
                                   'orderItemList': JSON.stringify(prods)
                                  });
                action1.setCallback(this,function(response){ 
                    if(response.getState() == "SUCCESS"){
   ...
   //Saving New Invoice-nanma
    saveOrderNew : function(component,event,helper) {  
         component.set('v.spinner', true);
        var data =   JSON.parse( JSON.stringify(component.get('v.data') ) );
          var prods = data.orderitems;
        var stockAvailable;
        ...
        if(stockAvailable == true){
            var action1=component.get("c.upsertNewOrderNew");
            action1.setParams({'orderList':  JSON.stringify(data.orders),
                               'accountId' : accId,
                               'orderItemList': JSON.stringify(data.orderitems)
                              });
            action1.setCallback(this,function(response){ 
  ...
  /*  saveNewInvoice: function(component,event,helper) {  
        ...
        if(stockAvailable == true){
              ...
              console.log('DATA : '+component.get('v.data'));
                ...
                var action1=component.get("c.saveInvoice");
                action1.setParams({'orderList':  JSON.stringify(data.orders),
                                   'orderItemList': JSON.stringify(prods),
                                   'accountId' : accId
                                  });
                action1.setCallback(this,function(response){ 
                    ...
                    if(response.getState() == "SUCCESS"){
                        ...
                        var oid = res[0].Sales_Invoice__c;
                        console.log('Invoice inserted=== ');
    ...
    //To get stock details for Order
    getManuFacturingDatesOrder : function(component,event,helper,cartProductId){
        console.log('cartProductId : '+cartProductId)
      
        return new Promise(
            $A.getCallback(function(resolve, reject) {
                var productStockMfgMap = [];
                var action=component.get("c.getCartProductStocksOrder");
                action.setParams({
    ...
    //To get stock details for Invoice
